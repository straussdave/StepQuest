...
Real-world walking progress is measured using the motion sensors available on Android devices. The prototype first attempts to use the Android \texttt{TYPE\_STEP\_COUNTER} sensor. This sensor reports the cumulative number of steps recorded since the device was last restarted. When a quest or tracking session begins, \textit{StepQuest} stores the current cumulative value as a baseline. Subsequent progress is calculated as the difference between the current sensor value and this baseline.
...
The final \textit{StepQuest} prototype implemented selected elements from the intended Octalysis and BCW mapping. The strongest representation was found in \textit{Epic Meaning and Calling}, \textit{Development and Accomplishment}, \textit{Ownership and Possession}, and \textit{Scarcity and Impatience}. Other core drives were implemented only partially or were excluded from the prototype.
