...
The prototype was divided into separate systems for step tracking, quest management, persistence, user interface control, content unlocking, and data logging. A high level overview is depicted in Figure~\ref{fig:software_architecture} This separation reduced dependencies between the physical activity sensor and the gameplay logic. For example, the quest system receives step updates through a common step-counter interface rather than directly accessing the Android sensor implementation.

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{PICs/stepQuestArchitecture.png}
	\caption{High-level software architecture of \textit{StepQuest}.}
...
Seven complete post-questionnaires were available for analysis. Table~\ref{tab:post_questionnaire_results} summarizes the response distributions for the closed Likert-scale items, while Figure~\ref{fig:post_questionnaire_results} represents the same data in more detail as a stacked horizontal bar chart.
...
\begin{table}[h]
	...
	\begin{tabular}{p{0.48\textwidth} c c c c}
		\hline
		\textbf{Item} & \textbf{Disagree} & \textbf{Neutral} & \textbf{Agree} \\
		\hline
		More motivated to walk & 4 & 0 & 3  \\
		Walking felt more meaningful or rewarding & 0 & 3 & 4  \\
		Daily quests encouraged more walking & 2 & 2 & 3  \\
		Wanted to continue the story and recover parts & 1 & 0 & 6  \\
		Connection between steps and game progress was clear & 0 & 0 & 7  \\
		Application was easy to understand & 0 & 0 & 7  \\
		Always knew what to do next & 0 & 0 & 7  \\
		Step-progress feedback was clear and useful & 1 & 0 & 6  \\
		Technical issues made the application harder to use & 3 & 1 & 3  \\
		Would use a more complete version again & 1 & 0 & 6 & \\
		Would recommend the concept & 0 & 0 & 7  \\
		\hline
	\end{tabular}
\end{table}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{PICs/postQuestionnaire.png}
	\caption{Distribution of responses to the post-questionnaire Likert-scale items ($n=7$).}
	\label{fig:post_questionnaire_results}
\end{figure}
...
Table~\ref{tab:usage_engagement_metrics} summarizes the cleaned usage metrics for the nine available log files. Quest completion was determined using unique quest unlock events rather than the automatically generated completion count, as duplicated completion events were identified in several logs.

\begin{table}[h]
	\centering
	\caption{Descriptive usage and engagement metrics derived from the cleaned application logs ($n=9$)}
	\label{tab:usage_engagement_metrics}
	\begin{tabular}{lrrrr}
		\hline
		\textbf{Participant} & \textbf{Active Days} & \textbf{Sessions} & \textbf{Quests Completed} & \textbf{Story Quests} \\
		\hline
		P01 & 15 & 25 & 12 & 4 \\
		P02 & 27 & 32 & 12 & 4 \\
		P03 & 21 & 50 & 12 & 4 \\
		...
		P09 & 8  & 12 & 2  & 2 \\
		\hline
		\textbf{Median} & \textbf{15} & \textbf{25} & \textbf{5} & \textbf{2} \\
		\textbf{Range} & \textbf{5--27} & \textbf{4--69} & \textbf{2--12} & \textbf{0--4} \\
		\hline
	\end{tabular}
\end{table}

Participants used the prototype on a median of 15 distinct days and recorded a median of 25 application sessions. The median number of uniquely completed quests was five, while four participants completed all twelve available quests. Considerable variation was observed across participants, particularly in the number of active days and completed quests.
